...
1. Run the container using `docker run -d --name=grafana -e "GF_PLUGINS_ALLOW_LOADING_UNSIGNED_PLUGINS=osisoft-sds-sample" -p 3000:3000 grafana-ocs`. The -e switch creates an [environment variable](https://grafana.com/docs/grafana/latest/administration/configuration/#configure-with-environment-variables) that overrides options in the grafana config. In this case, the `allow_loading_unsigned_plugins` option is being overridden to allow the unsigned plugin in this sample to be used.
